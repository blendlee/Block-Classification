...
def augmentation(df):
    ...
    for img_path,id in tqdm(zip(df['img_path'],df['id'])):
        ...
        image = Image.open('data/train_rembg/'+file_name+'.png')
        image = np.array(image)
        image = transform(image=image)['image']
        pil_image=Image.fromarray(image)
        aug_path = 'data/rembg_aug/'+file_name+'.png'
        aug_image_path.append('.'+aug_path[4:])
...
#'./augmentation/train/TRAIN_10108.jpg
    ...
    df.to_csv('data/train_rembg_aug.csv',index=False)
